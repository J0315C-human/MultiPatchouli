...
void EnvFollower::DacCallback(uint16_t **output, size_t size)
{
    float knob_attackMS  = patch.GetAdcValue(CV_1);
    float knob_releaseMS = patch.GetAdcValue(CV_2);
    envelopeScaling      = fmap(patch.GetAdcValue(CV_3), 0.f, 3.f);
    makeupGain_R         = fmap(patch.GetAdcValue(CV_4), 0.f, 2.f);
